...
export default async function PrivacyPage() {
  ...
  const [
    ...
    supabase.from('pages').select('*').eq('slug', 'privacy').eq('is_published', true).single(),
    supabase.from('site_content').select('value').eq('key', 'social'),
    supabase.from('site_content').select('value').eq('key', 'nav'),
  ])

  const content = pageRes?.content || 'Privacy Policy coming soon.'
    ...
    <>
      ...
      <main className="pt-32 pb-20 px-4">
        <div className="max-w-3xl mx-auto">
          <h1 className="font-display text-4xl text-[var(--color-text)] uppercase mb-8">{pageRes?.title || 'Privacy Policy'}</h1>
